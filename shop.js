    ...
    container.innerHTML = `
        ...
        <div class="dashboard-grid">
            <div class="form-panel">
                ...
                <input type="number" id="p-prix" placeholder="Ex: 350000" min="500">
    ...
    mesProduits.forEach(p => {
        container.innerHTML += `
            <div class="inventory-item">
                <div class="item-info">
                    <h4>${p.nom_produit}</h4>
                    <p>${p.prix.toLocaleString()} GNF <span style="color:#cbd5e1; font-weight:normal; margin:0 5px;">|</span> <small style="color:var(--text-light)">${p.categorie || 'Général'}</small></p>
                </div>
...
async function ajouterProduitReal(event) {
    ...
    // --- SÉCURITÉ PRIX ---
    if (!nom || isNaN(prix)) return alert("Veuillez remplir le nom et le prix.");
    if (prix < 100) return alert("Le prix semble trop bas. Veuillez vérifier.");
    ...
    const { error } = await _supabase.from('produits').insert([{ 
        nom_produit: nom, 
        prix: prix, 
        categorie: categorie, // Nouvelle colonne
        zone: zone, 
    ...
    if (!error) {
        ...
        alert("Erreur : " + error.message + "\nAssurez-vous que la colonne 'categorie' existe dans Supabase.");
    }
    btn.disabled = false;
...
Le scripte css fournisseur :

/* --- STYLE SCANNER FOURNISSEUR --- */
        #reader {
            width: 100%;
            border-radius: 15px;
            overflow: hidden;
